...
export function getPilotName(): string {
  const existingName = localStorage.getItem("space-system-pilot-name");

  if (existingName) {
    return existingName;
  }

  const name = `Pilot ${Math.floor(100 + Math.random() * 900)}`;
  localStorage.setItem("space-system-pilot-name", name);
  return name;
}
