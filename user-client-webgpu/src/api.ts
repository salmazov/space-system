...
export function getClientSession(): ClientSession {
  const existingClientId = sessionStorage.getItem("space-system-webgpu-client-id");
  const existingName = sessionStorage.getItem("space-system-webgpu-pilot-name");
  ...
  const session: ClientSession = {
    ...
    pilotName: `WebGPU Pilot ${Math.floor(100 + Math.random() * 900)}`
  };
  sessionStorage.setItem("space-system-webgpu-client-id", session.clientId);
  sessionStorage.setItem("space-system-webgpu-pilot-name", session.pilotName);
  return session;
}

function randomClientId(): string {
  const randomPart = globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.floor(Math.random() * 1_000_000)}`;
  return `webgpu-${randomPart}`;
}
