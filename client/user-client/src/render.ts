...
export function renderWorld(world: WorldSnapshot, elements: UserClientElements, clientId: string): void {
  ...
  syncOptions(
    ...
    Object.values(world.shipClasses).map((shipClass) => [
      shipClass.id,
      `${shipClass.label} - EUR ${shipClass.priceEuro} - fuel ${shipClass.startingFuel}/${shipClass.fuelCapacity}`
    ])
  );
...
function renderShip(world: WorldSnapshot, elements: UserClientElements, player: PlayerShip | null): void {
  ...
  elements.shipStats.replaceChildren(
    ...
    statRow("Fuel", `${formatNumber(player.fuel)}/${formatNumber(player.fuelCapacity)} (${player.fuelBurnPerUnit}/unit)`),
